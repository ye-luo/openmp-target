...
int main() {
  ...
  #pragma omp target enter data device(default_device) map(alloc : hst_ptr[: N])

  for (int i = 0; i < N; ++i)
    hst_ptr[i] = 2;

  #pragma omp target teams distribute parallel for device(default_device) map(always, tofrom : hst_ptr[: N])
  for (int i = 0; i < N; ++i)
    hst_ptr[i] -= 1;
  ...
  #pragma omp target exit data device(default_device) map(delete : hst_ptr[: N])
